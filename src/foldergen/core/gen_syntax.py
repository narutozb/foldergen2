...
def _expand_one(expr: str) -> list[str]:
    ...
    if typ == "int":
        ...
        start = kv.get("start"); stop = kv.get("stop")
        ...
        # 实际生成
        cur = date(y1,m1,d1); stop = date(y2,m2,d2)
        unit = step[-1].lower(); n = int(step[:-1])
        out = []
